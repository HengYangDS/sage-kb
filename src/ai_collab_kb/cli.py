...
@app.command()
def get(
    layer: int = typer.Argument(0, help="Layer (0=core, 1=guidelines, 2=frameworks, 3=practices)"),
    topic: Optional[str] = typer.Option(None, "--topic", "-t", help="Specific topic or task"),
    format: str = typer.Option("markdown", "--format", "-f", help="Output format: markdown, syntax, raw"),
...
@app.command()
def guidelines(
    section: str = typer.Argument("overview", help="Section: quick_start, planning, code_style, engineering, documentation, python, ai_collaboration, cognitive, quality, success"),
...
@app.command()
def framework(
    name: str = typer.Argument(..., help="Framework: autonomy, cognitive, decision, collaboration, timeout"),
    timeout: int = typer.Option(5000, "--timeout", help="Timeout in milliseconds"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show detailed information"),
):
    """
...
def validate(
    ...
    console.print(Panel(f"Validating: {kb_path}", title="Validation", border_style="blue"))
    ...
    console.print()
    if issues:
        console.print(f"[yellow]⚠ {len(issues)} issues found, {checks_passed} checks passed[/yellow]")
        if fix:
    ...
    while True:
        try:
            ...
            if action == "exit" or action == "quit":
                ...
                result = run_async(loader.load(
                    layer={0: Layer.L1_CORE, 1: Layer.L2_GUIDELINES, 2: Layer.L3_FRAMEWORKS}.get(layer, Layer.L1_CORE)
                ))
                ...
                stats = loader.get_cache_stats()
                console.print(f"Cached: {stats['cached_files']} files, {stats['total_size']:,} bytes")
                ...
                console.print(f"[yellow]Unknown command: {action}. Type 'help' for commands.[/yellow]")
