---
version: "1.0"
last_updated: "2025-11-30"
status: published
tokens: ~1150
---

# Expert Committee Framework

> Multi-perspective analysis using 35+ expert roles

---

## Table of Contents

- [1. Expert Role Pool](#1-expert-role-pool)
- [2. Committee Patterns](#2-committee-patterns)
- [3. Selection Process](#3-selection-process)
- [4. Analysis Process](#4-analysis-process)
- [5. Output Format](#5-output-format)
- [6. Quick Reference](#6-quick-reference)

---

## 1. Expert Role Pool

### 1.1 Technical Experts (12)

| Role                 | Focus                       |
|----------------------|-----------------------------|
| Software Architect   | System design, scalability  |
| Security Engineer    | Vulnerabilities, compliance |
| Performance Engineer | Optimization, benchmarks    |
| Database Specialist  | Data modeling, queries      |
| DevOps Engineer      | CI/CD, infrastructure       |
| API Designer         | Interface contracts         |
| Frontend Specialist  | UX, accessibility           |
| Backend Specialist   | Business logic              |
| Data Engineer        | Pipelines, ETL              |
| ML Engineer          | Models, training            |
| QA Engineer          | Testing strategies          |
| Tech Lead            | Trade-offs, priorities      |

### 1.2 Domain Experts (12)

| Role               | Focus                 |
|--------------------|-----------------------|
| Product Manager    | Requirements, roadmap |
| Business Analyst   | Process, value        |
| Domain Expert      | Industry knowledge    |
| UX Designer        | User experience       |
| Technical Writer   | Documentation         |
| Project Manager    | Timeline, resources   |
| Compliance Officer | Regulations           |
| Support Engineer   | User issues           |
| Release Manager    | Deployment            |
| Stakeholder Rep    | Business needs        |
| End User Rep       | Usability             |
| Operations         | Maintenance           |

### 1.3 Strategic Experts (11)

| Role                 | Focus              |
|----------------------|--------------------|
| CTO                  | Technical strategy |
| Enterprise Architect | Integration        |
| Risk Manager         | Risk assessment    |
| Innovation Lead      | Future tech        |
| Cost Analyst         | Budget, ROI        |
| Change Manager       | Adoption           |
| Knowledge Manager    | Documentation      |
| Quality Director     | Standards          |
| Security Director    | Security posture   |
| Data Privacy Officer | Privacy            |
| Sustainability Lead  | Long-term impact   |

---

## 2. Committee Patterns

| Pattern           | Experts | Use Case           |
|-------------------|---------|--------------------|
| **Quick Review**  | 3       | Simple decisions   |
| **Standard**      | 5-7     | Normal complexity  |
| **Comprehensive** | 9-12    | Major decisions    |
| **Full Board**    | 15+     | Critical/strategic |

---

## 3. Selection Process

### 3.1 By Problem Type

| Problem Type | Key Experts                      |
|--------------|----------------------------------|
| Architecture | Architect, Security, Performance |
| Feature      | Product, UX, Tech Lead           |
| Bug fix      | QA, Domain Expert, Support       |
| Security     | Security, Compliance, Risk       |
| Performance  | Performance, DBA, DevOps         |

### 3.2 Selection Criteria

1. Relevance to problem domain
2. Complementary perspectives
3. Potential conflict identification
4. Decision authority level

---

## 4. Analysis Process

### 4.1 Individual Analysis

Each expert provides:

- Perspective on the problem
- Key concerns from their domain
- Recommendations
- Risk assessment

### 4.2 Synthesis

| Step | Action                   |
|------|--------------------------|
| 1    | Collect all perspectives |
| 2    | Identify agreements      |
| 3    | Highlight conflicts      |
| 4    | Weigh trade-offs         |
| 5    | Form recommendation      |

---

## 5. Output Format

```markdown
## Expert Committee Analysis

### Participants
[List of experts consulted]

### Key Findings
[Synthesized insights]

### Recommendations
[Prioritized actions]

### Dissenting Views
[Alternative perspectives]

### Decision
[Final recommendation with rationale]
```

---

## 6. Quick Reference

| Committee Size | Time     | Use For             |
|----------------|----------|---------------------|
| 3 experts      | 15 min   | Quick validation    |
| 5-7 experts    | 30 min   | Standard analysis   |
| 9-12 experts   | 1 hour   | Deep analysis       |
| 15+ experts    | Half day | Strategic decisions |

---

## Related

- `.knowledge/templates/expert_committee.md` — Decision templates
- `.knowledge/frameworks/patterns/decision.md` — Quality angles
- `.knowledge/guidelines/cognitive.md` — Cognitive guidelines

---

*Part of SAGE Knowledge Base*
