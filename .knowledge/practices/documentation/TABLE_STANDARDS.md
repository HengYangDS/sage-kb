...
**术 (Technique)** — Example of status indicators:
...
|----------------|---------------------------------|--------------------------------------|
| **Comparison** | Compare items across attributes | Feature matrices, option evaluation  |
| **Reference**  | Quick lookup of values          | API docs, config options             |
| **Mapping**    | Show relationships              | Error codes, status definitions      |
| **Summary**    | Aggregate information           | Metrics, dashboards                  |
| **Checklist**  | Track completion                | Progress tracking, quality checks    |
...
Use `:` in the separator row to control alignment: `:---` (left), `:---:` (center), `---:` (right):
...
**Example split** — divide a large parameter table into focused sections:
...
Use for feature comparisons across multiple items (center-align data columns for visual balance):
...
Use icons (✅ ⚠️ ❌) for quick visual scanning:
...
Use for documenting choices with weighted criteria. Use fixed-width symbols (●○) for consistent alignment:
...
Use `<br>` for line breaks within cells to show multiple values:
...
Use `[text](url)` links to connect to detailed documentation:
